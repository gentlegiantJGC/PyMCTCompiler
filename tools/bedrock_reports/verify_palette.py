...
def main(path):
    for dump_file_path in glob.glob(os.path.join(path, '**', 'block_id_to_item_id.json'), recursive=True):
        block_palette_path = os.path.join(os.path.dirname(dump_file_path), 'block_item_palette.json')
        if os.path.isfile(block_palette_path):
            ...
            with open(os.path.join(os.path.dirname(dump_file_path), 'block_item_palette_missing.txt'), 'w') as f:
                f.write('\n'.join(missing_blocks))
