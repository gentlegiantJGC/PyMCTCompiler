...
def main(path):
    for dump_file_path in glob.glob(
        os.path.join(path, "**", "item_id.txt"), recursive=True
    ):
        ...
        for item_id in item_ids:
            if item_id:
                ...
                if slot == 27:
                    with open(
                        os.path.join(spawn_path, f"chest{x}_{z}.mcfunction"), "w"
                    ) as f:
                        f.write("\n".join(commands))
                    main_commands.append(
                        f"execute @a[scores={{t={(x * 16 + z) // 16}}}] ~ ~ ~ function spawn_{version}/chest{x}_{z}"
                    )
                    commands.clear()
