...
def main(path):
    for dump_file_path in glob.glob(os.path.join(path, '**', 'block_id_to_item_id.json'), recursive=True):
        print(dump_file_path)

        with open(dump_file_path) as f:
            block_ids = [i.split(':', 1)[1] for i in json.load(f).keys()]

        version = '_'.join(os.path.split(os.path.dirname(os.path.dirname(dump_file_path)))[1].split('_')[1:3])
        ...
        for block in block_ids:
            ...
            for slot in range(27):
                commands.append(f"replaceitem block {x} 100 {z} slot.container {slot + 27 * (x % 2)} {block} 1 {slot}")
            ...
            with open(os.path.join(os.path.join(replaceitem_path, block) + '.mcfunction'), 'w') as f:
                f.write('\n'.join(commands))
            main_commands.append(f'execute @a[scores={{t={x * 16 + z + 1}}}] ~ ~ ~ function replaceitem_{version}/{block}')
