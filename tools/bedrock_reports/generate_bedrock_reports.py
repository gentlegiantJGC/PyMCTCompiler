...
matches = {
    "block_entity_id": re.compile(b'@V(?P<match>[._a-zA-Z0-9]+?)BlockActor@@@std@@\0'),  # The name identifier used in the block entity
    "entity_id": re.compile(b'entity\.(?P<match>[._a-zA-Z0-9]+?)\.name\0'),  # The name identifier used in the block entity
            ...
            for line in f.readlines():
                for match_name, match in matches.items():
                    for s in match.finditer(line):
                        matched_strings[match_name].add(s.group('match').decode('utf-8'))
