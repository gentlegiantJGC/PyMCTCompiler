...
def get(primitive_group: Union[str, List[str], List[List[str]]], parents: list, feature_set: Set[str] = None) -> FunctionList:
	...
	if primitive_group in nested_primitives:
		...
		if isinstance(primitive_group, str):
			...
			assert len(primitive_group) > 0, 'There was no data in the requested primitive group'
