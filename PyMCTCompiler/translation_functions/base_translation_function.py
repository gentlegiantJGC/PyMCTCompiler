...
class FunctionList(BaseTranslationObject):
    def __init__(self, data, instant_commit=False):
        ...
        for fun in data:
            if fun["function"] in translation_functions.function_map:
                self.function_list.append(
                    translation_functions.function_map[fun["function"]](fun)
                )
            else:
    ...
    def _primitive_extend(self, other: "FunctionList", parents: list):
        ...
        # ]
        self_functions = [
            [fun.function_name, fun.custom_name] for fun in self.function_list
        ]
        other_functions = [
            [fun.function_name, fun.custom_name] for fun in other.function_list
        ]
    ...
    def _compiled_extend(self, other, parents: list):
        ...
        """
        assert [fun.function_name for fun in self.function_list] == [
            fun.function_name for fun in other.function_list
    ...
    def commit(self, feature_set: Union[Set[str], None], parents: list):
        ...
        if feature_set is None:
            feature_set = translation_functions.default_feature_set
    ...
    def _commit(self, feature_set: Set[str], parents: list):
        for fun in self.function_list:
            assert (
                fun.function_name in feature_set
            ), f'Function "{fun.function_name}" is not valid here.'
            fun.commit(feature_set, parents)

    def save(self, parents: list) -> list:
        if self._is_primitive:
            raise Exception(
                "The commit function must be called before save is called to confirm the format is valid"
            )
