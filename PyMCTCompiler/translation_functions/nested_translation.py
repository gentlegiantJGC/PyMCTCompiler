...
class NestedTranslation(BaseTranslationFunction):
    ...
    def __init__(self, data):
        from PyMCTCompiler import (
            primitives,
        )  # if this is imported at the top it causes issues because values have not been defined
    ...
    def _primitive_extend(self, other: BaseTranslationFunction, parents: list):
        ...
        self["options"][0] = self["options"][0] + [
            op for op in other["options"][0] if op not in self["options"][0]
        ]
    ...
    def _compiled_extend(self, other: BaseTranslationFunction, parents: list):
        ...
        self["options"] = self["options"] + [
            op for op in other["options"] if op not in self["options"]
        ]
