...
class CarryProperties(BaseTranslationFunction):
    ...
    def _primitive_extend(self, other: BaseTranslationFunction, parents: list):
        ...
        for prop, val in other["options"]:
            if prop not in self["options"]:
                ...
                self["options"][prop] = self["options"][prop] + [
                    v for v in val if v not in self["options"][prop]
                ]
    ...
    def _compiled_extend(self, other: BaseTranslationFunction, parents: list):
        ...
        ), f'"carry_properties" must have the same key entries when merging\n{self["options"].keys()}\n{other["options"].keys()}'
        for key in other["options"].keys():
            self["options"][key] = self["options"][key] + [
                k for k in other["options"][key] if k not in self["options"][key]
            ]
    ...
    def _commit(self, feature_set: Set[str], parents: list):
        ...
        for key, val_list in self["options"].items():
            ...
            assert isinstance(
                val_list, list
            ), '"options" values must be a list of strings'
            for val in val_list:
