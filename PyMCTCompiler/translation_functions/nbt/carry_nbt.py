...
class CarryNBT(BaseTranslationFunction):
    ...
    def _compiled_extend(self, other: BaseTranslationFunction, parents: list):
        ...
        assert (
            self["options"] == other["options"]
        ), '"carry_nbt" must be the same when merging'

    def _commit(self, feature_set: Set[str], parents: list):
        assert isinstance(self["options"], dict), "options must be a dictionary"
        if "path" in self["options"]:
            assert isinstance(
                self["options"]["path"], list
            ), '"options" path must be a list of lists'
            for index, path in enumerate(self["options"]["path"]):
                assert isinstance(path, list), '"options" path must be a list of lists'
                assert (
                    len(path) == 2
                ), '"options" path must be a list of lists of length 2'
                if index == 0:
                    assert isinstance(
                        path[0], str
                    ), '"new_nbt" path entry [0][0] must be a string because it is wrapped in an implied compound tag'
                else:
                    if isinstance(path[0], str):
                        assert (
                            self["options"]["path"][index - 1][1] == "compound"
                        ), f'Expected the previous data type to be "compound" got {path[index - 1][1]}'
                    elif isinstance(path[0], int):
                        assert (
                            self["options"]["path"][index - 1][1] == "list"
                        ), f'Expected the previous data type to be "list" got {path[index - 1][1]}'
                    else:
                        raise Exception(
                            "The first paramater of each entry in path must be a string or an int"
                        )
        ...
        if "key" in self["options"]:
            if isinstance(self["options"]["key"], str):
                if "path" in self["options"]:
                    assert (
                        self["options"]["path"] == []
                        or self["options"]["path"][-1][1] == "compound"
                    ), f'Expected the final data type in path to be "compound" got {self["options"]["path"][-1][1]}'
            elif isinstance(self["options"]["key"], int):
                if "path" in self["options"]:
                    assert (
                        self["options"]["path"][-1][1] == "list"
                    ), f'Expected the final data type in path to be "list" got {self["options"]["path"][-1][1]}'
            else:
                raise Exception(
                    "The first paramater of each entry in path must be a string or an int"
                )
