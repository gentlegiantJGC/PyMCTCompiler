        ...
        for new_nbt in new_nbts:
            ...
            if "path" in new_nbt:
                ...
                for index, path in enumerate(new_nbt["path"]):
                    ...
                    assert (
                        len(path) == 2
                    ), '"new_nbt" path must be a list of lists of length 2'
                    if index == 0:
                        assert isinstance(
                            path[0], str
            ...
            if isinstance(new_nbt["key"], str):
                ...
                if "path" in new_nbt:
                    assert (
                        new_nbt["path"][-1][1] == "list"
                    ), f'Expected the final data type in path to be "list" got {new_nbt["path"][-1][1]}'
            else:
                raise Exception(
                    "The first paramater of each entry in path must be a string or an int"
                )
