...
def _minify_block(block: dict):
    return {
        ...
        "defaults": next(
            state for state in block["states"] if state.get("default", False)
        ).get("properties", {}),
    }
        ...
        if block not in new_blocks:
            ...
            for prop, prop_data in block_data["properties"].items():
                if prop not in new_block_data["properties"]:
                    changes.setdefault(block, {}).setdefault(
                        "properties_removed", []
                    ).append(prop)
                else:
        ...
        if block not in old_blocks:
            ...
            for prop, prop_data in block_data["properties"].items():
                if prop not in old_block_data["properties"]:
                    changes.setdefault(block, {}).setdefault("properties_added", {})[
                        prop
                    ] = prop_data
                else:
                    if block_data["defaults"][prop] != old_block_data["defaults"][prop]:
                        changes.setdefault(block, {}).setdefault("default_changed", {})[
                            prop
                        ] = [
                            old_block_data["defaults"][prop],
                            block_data["defaults"][prop],
                        ]
                    for val in prop_data:
...
class JavaBlockstateCompiler(BaseCompiler):
    ...
    def _build_blocks(self):
        ...
        blocks_path = os.path.join(
            self._directory, "generated", "reports", "blocks.json"
        )
        if os.path.isfile(blocks_path):
            ...
            for (namespace, sub_name), block_data in self.blocks.items():
                ...
                for block_base_name, primitive_data in block_data.items():
                    ...
                    add[(namespace, sub_name)][block_base_name] = primitives.get_block(
            ...
            parent_blocks_path = os.path.join(
                self._directory,
                "..",
                self._parent_name,
                "generated",
                ...
                if not (namespace in remove and block_base_name in remove[namespace]):
                    ...
                    if "properties" in default_state:
                        ...
                        from_universal = FunctionList(
                            [
                                {"function": "new_block", "options": block_string},
                                {
                                    "function": "carry_properties",
                                    "options": states["properties"],
                                },
                        ...
                        from_universal = FunctionList(
                            [{"function": "new_block", "options": block_string}], True
                        )
            ...
            for namespace, sub_name in add:
                for block_base_name, block_data in add[(namespace, sub_name)].items():
                    if disk_buffer.has_translation_to_universal(
                        ...
                        assert (
                            "to_universal" in block_data
                        ), f'"to_universal" must be present. Was missing for {self.version_name} {namespace}:{block_base_name}'
                        disk_buffer.add_translation_to_universal(
                            self.version_name,
                            "block",
                            "blockstate",
        ...
        for (namespace, sub_name), entity_data in self.entities.items():
            for entity_base_name, primitive_data in entity_data.items():
                ...
                if disk_buffer.has_translation_to_universal(
                    ...
                    assert (
                        "specification" in entity_primitive_file
                    ), f'"to_universal" must be present. Was missing for {self.version_name} {namespace}:{entity_base_name}'
                    ...
                    assert (
                        "to_universal" in entity_primitive_file
                    ), f'"to_universal" must be present. Was missing for {self.version_name} {namespace}:{entity_base_name}'
                    disk_buffer.add_translation_to_universal(
                        self.version_name,
                        "entity",
                        ...
                        sub_name,
                        entity_base_name,
                        entity_primitive_file["to_universal"],
                    )

                    assert (
                        "from_universal" in entity_primitive_file
                    ), f'"to_universal" must be present. Was missing for {self.version_name} {namespace}:{entity_base_name}'
                    for entity_string2, mapping in entity_primitive_file[
                        "from_universal"
                    ].items():
