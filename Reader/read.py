...
class Namespace:
	...
	def to_universal(self, level, block_name: str, properties: Dict[str, str]):
		blockstate = {"block_name": f"{self.namespace}/{block_name}", "properties": properties}
		try:
			blockstate = self.convert(
				level,
				blockstate,
	...
	def convert(self, level, input_blockstate: dict, mappings: dict, output_version: SubVersion):
		"""
		...
		"""
		output_blockstate, new = self._convert(level, input_blockstate, mappings, output_version)
		for entry in ["properties", "nbt"]:
			for key, val in new[entry].items():
				output_blockstate[entry][key] = val
		return output_blockstate

	def _convert(self, level, input_blockstate: dict, mappings: dict, output_version: SubVersion):
			...
			for key in mappings["map_properties"]:
				if key in input_blockstate["properties"]:
					...
					if val in mappings["map_properties"][key]:
						temp_blockstate, temp_new = self._convert(level, input_blockstate, mappings["map_properties"][key][val], output_version)
